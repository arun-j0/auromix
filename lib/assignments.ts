...
export const createAssignment = async (assignmentData: Omit<Assignment, "id" | "createdAt" | "updatedAt">) => {
  try {
    ...
    await setDoc(assignmentRef, {
      ...
      approvedAt: assignment.approvedAt ? Timestamp.fromDate(assignment.approvedAt) : null,
      rejectedAt: assignment.rejectedAt ? Timestamp.fromDate(assignment.rejectedAt) : null,
      startedAt: assignment.startedAt ? Timestamp.fromDate(assignment.startedAt) : null,
...
export const getAssignmentsByAgent = async (agentId: string): Promise<Assignment[]> => {
  try {
...
export const approveAssignment = async (assignmentId: string, approvedBy: string) => {
  try {
...
export const rejectAssignment = async (assignmentId: string, rejectedBy: string, rejectionReason: string) => {
  try {
    await updateAssignment(assignmentId, {
      status: "rejected",
      rejectedAt: new Date(),
...
export const completeAssignment = async (assignmentId: string, notes?: string) => {
  try {
    await updateAssignment(assignmentId, {
      status: "completed",
